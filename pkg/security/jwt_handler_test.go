...
func TestJwtHandler_GetTokenTtl(t *testing.T) {
	const expectedTtl = time.Second * 42

	jh := getReferenceJwtHandler()
	jh.TokenTtl = expectedTtl
	up := getReferenceUser()
	...
	// actual remaining TTL from token will be less than the predefined TTL constant because some time has passed since
	// generating it. Also, there is an effect caused by rounding time during the serialization to JWT
	ttlFromToken := time.Until(payload.ExpiresAt)
	//t.Log("ttlFromToken", ttlFromToken)
	diff := ttl - ttlFromToken
	diff = diff.Abs()
